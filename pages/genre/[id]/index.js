...
const index = ({ genreRes }) => (
  <div className="flex justify-center pt-20 mx-40">
    <div className="grid grid-cols-2 gap-4 ">
      {
        genreRes.results
        && genreRes.results.map((items) => {
          const {
            mal_id, synopsis, title, image_url, episodes, airing_start, score,
          } = items;
          return (
...
export const getServerSideProps = async (context) => {
  const res = await fetch(`https://animex-backend.herokuapp.com/api/anime/search/genre/${context.params.id}`);
